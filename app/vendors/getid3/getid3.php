...
$temp_dir = TMP; //ini_get('upload_tmp_dir');
	...
	foreach (get_included_files() as $key => $val) {
		if (basename($val) == 'getid3.php') {
			//define('GETID3_INCLUDEPATH', dirname($val).DIRECTORY_SEPARATOR);
                    define('GETID3_INCLUDEPATH', APP.'vendors/getid3/');
...
//						),
	...
	public function include_module($name) {
		...
		if (!file_exists(GETID3_INCLUDEPATH.'module.'.$name.'.php')) {
			throw new getid3_exception('Required module.'.GETID3_INCLUDEPATH.$name.'.php is missing.');
		}
